...
function zcartesianProduct( order, M, N, x, strideX, y, strideY, out, LDO ) {
	...
	if ( l === null ) {
		throw new TypeError( format( '34rFx', order ) );
	}
	if ( isColumnMajor( l ) ) {
		if ( LDO < max( 1, M*N ) ) {
			throw new RangeError( format( '34rGi', M*N, LDO ) );
		}
	} else if ( LDO < 2 ) { // order === 'row-major'
		throw new RangeError( format( '34rGi', 2, LDO ) );
	}
	xView = reinterpret( x, 0 );
	yView = reinterpret( y, 0 );
	oView = reinterpret( out, 0 );
	addon( resolveOrder( order ), M, N, xView, strideX, yView, strideY, oView, LDO ); // eslint-disable-line max-len
	return out;
}
